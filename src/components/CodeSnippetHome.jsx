...
export default function CodeSnippetHome() {
  ...
  const handleDeleteSnippet = async () => {
    ...
    try {
      console.log(`Attempting to delete snippet with id: ${snippetToDelete.id}`);
      const response = await fetch(`http://localhost:3000/snippets/${snippetToDelete.id}`, {
        method: 'DELETE',
      });
                    ...
                    <CardContent sx={{ flexGrow: 1, pt: 4, pb: 2 }}>
                      ...
                      <Typography 
                        ...
                        sx={{ 
                          ...
                          backgroundColor: 
                            snippet.difficulty === 'Easy' ? 'rgba(46, 204, 113, 0.2)' : 
                            snippet.difficulty === 'Medium' ? 'rgba(241, 196, 15, 0.2)' : 
                            'rgba(231, 76, 60, 0.2)',
                          color: 
                            snippet.difficulty === 'Easy' ? '#2ecc71' : 
                            snippet.difficulty === 'Medium' ? '#f1c40f' : 
                            '#e74c3c',
                          px: 1.5,
                          py: 0.5,
                          borderRadius: 1,
