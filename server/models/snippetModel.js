...
export const getAllSnippets = async() => {
    const { rows } = await pool.query("SELECT * FROM snippets ORDER BY created_at DESC"); // Example: Can be modified
    return rows;
}
...
export const createSnippet = async ({ title, description, difficulty, code, }) => {
...
export const updateSnippet = async (id, { title, description, difficulty, code }) => {
    const { rows } = await pool.query(
...
// Delete a snippet
export const deleteSnippet = aync (id) => {
